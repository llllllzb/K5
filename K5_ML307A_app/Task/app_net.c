...
{
    char param[100];
    sprintf(param, "%d,\"TCP\",\"%s\",%d,60,3,0", link, server, port);
...
void netConnectTask(void)
{
    ...
    switch (moduleState.fsmState)
    {
        ...
        case CONFIG_STATUS:
        	...
        	sendModuleCmd(CNMI_CMD, "2,1");						/*µÚ¶þ¸ö²ÎÊý±íÊ¾»º´æÔÚMEÖÐ, ²»Á¢¼´ÉÏ±¨*/
...
static void miprdParser(uint8_t *buf, uint16_t len)
{
	...
	while (index >= 0)
	{
		...
		LogPrintf(DEBUG_ALL, "index1:%d", index);
		...
		relen -= index;
		index = my_getstrindex(rebuf, "+MIPRD:", relen);
		LogPrintf(DEBUG_ALL, "index2:%d", index);
	}
}
